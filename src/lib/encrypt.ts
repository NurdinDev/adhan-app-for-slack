...
export class Encrypted {
  ...
  #password = env.get('CRYPTO_SECRET').required().asString();
  #salt = env.get('CRYPTO_SALT').required().asString();
  #secret = env.get('CRYPTO_SECRET').required().asString();
  ...
  encodeInformation = <T>(info: T): string => {
    ...
    const cipheredData =
      cipher.update(JSON.stringify(envelop), 'utf8', 'hex') +
      cipher.final('hex');
    return cipheredData;
  };

  decodeInformation = <T>(cipheredData: string): T | null => {
    const decipher = crypto.createDecipheriv(this.algorithm, this.key, this.iv); // 復号用インスタンス
    const decipheredDataJson =
      decipher.update(cipheredData, 'hex', 'utf8') + decipher.final('utf8');
